...
def main(config):
    ...
    model.load_state_dict(torch.load(os.path.join('learnt_focus', 'results', config['RUN_ID'], 'best_acc.pt')))
    ...
    save_folder = os.path.join('learnt_focus', "results", config['RUN_ID'], 'evaluation_' + config['DATASET'] + masking_str)
    try:
